...
Examining the performance entry, **we can see the "*dataSource*" property shows the XmlHttpRequest URL**. However, it doesn't show the source of the call which is needed to better understand how and why the request was made. **For that, we need to find and examine KPIs such as FullLoad or SaveForm.**
